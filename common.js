...
async function initSiteTitle(){
  const els = [...document.querySelectorAll('#siteTitle,.site-brand')];
  if(!els.length) return;

  const fallback = '自陣キャラ図鑑';
  const apply = title => {
    const safeTitle = (title || fallback).trim() || fallback;
    els.forEach(x => x.textContent = safeTitle);
    document.title = document.title ? document.title : safeTitle;
  };

  try{
    const r = await sb.from('site_settings').select('value').eq('key','site_title').maybeSingle();
    if(!r.error && r.data && r.data.value) apply(r.data.value);
  }catch(error){
    console.warn('site title load skipped', error);
  }
  ...
  els.forEach(el => {
    ...
    el.addEventListener('click', async () => {
      ...
      if(next === null) return;
      const title = next.trim() || fallback;
      apply(title);

      const r = await sb.from('site_settings').upsert({
        key: 'site_title',
        value: title,
      ...
      if(r.error){
        alert('タイトル保存に失敗しました：' + r.error.message);
